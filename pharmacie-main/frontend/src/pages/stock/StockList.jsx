    ...
    (async () => {
      try {
        ...
        const result = await fetchStock({
          ...
          emplacement_id: parsePositiveInt(
            searchParams.get("emplacement_id") || "",
          ),
          page: currentPage,
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-1">
                ...
                <h2 className="text-2xl font-semibold tracking-tight text-slate-900">
                  {t("page.stock.subtitle")}
                </h2>
                <p className="max-w-3xl text-sm leading-6 text-muted-foreground">
                  The page is organized for quick checks first: filter by
                  product or location, then expand lots only when needed.
                </p>
              </div>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <div className="rounded-2xl border bg-muted/20 p-4">
              ...
              <p className="mt-3 text-sm font-medium text-slate-900">
                {productId || t("common.all")}
              </p>
              <p className="text-sm text-muted-foreground">
                {t("stock.summary.product")}
              </p>
            </div>
            <div className="rounded-2xl border bg-muted/20 p-4">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <p className="mt-3 text-sm font-medium text-slate-900">
                {emplacementId || t("common.all")}
              </p>
              <p className="text-sm text-muted-foreground">
                {t("stock.summary.location")}
              </p>
            </div>
          </div>
      ...
      {!loading && !error && items.length > 0 && (
        <>
                      ...
                      <tr className="border-t hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <div className="font-medium text-slate-900">
                            {item.product_lib || t("common.notAvailable")}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            {item.product_id ?? t("common.notAvailable")}
                          </div>
                        </td>
                        <td className="px-4 py-3 text-muted-foreground">
                          {item.product_bar_code || t("common.notAvailable")}
                        </td>
                        <td className="px-4 py-3">{item.quantity}</td>
                        <td className="px-4 py-3">
                          {item.locker || t("common.notAvailable")}
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <Button
                            ...
                            variant="outline"
                            className="rounded-xl"
                            onClick={() => toggleLotsDetails(item)}
                          >
                            {openedStockId === item.stock_id
                              ? t("stock.hideLots")
                              : t("stock.viewLots")}
                          </Button>
                        </td>
                                      ...
                                      <thead className="bg-muted/50">
                                        <tr className="text-left">
                                          <th className="px-3 py-2 text-xs font-semibold">
                                            {t("stockLots.stockLotId")}
                                          </th>
                                          <th className="px-3 py-2 text-xs font-semibold">
                                            {t("stockLots.lotId")}
                                          </th>
                                          <th className="px-3 py-2 text-xs font-semibold">
                                            {t("stockLots.lotLabel")}
                                          </th>
                                          <th className="px-3 py-2 text-xs font-semibold">
                                            {t("stockLots.state")}
                                          </th>
          ...
          <Pagination
            ...
            onPrevious={() =>
              updatePagination(Math.max(1, pagination.page - 1))
            }
            onNext={() =>
              updatePagination(
