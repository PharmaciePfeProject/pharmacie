...
export const PickerField = ({
  ...
  return (
    <div className="relative">
      ...
      {open ? (
        <div className="absolute left-0 top-[calc(100%+0.5rem)] z-50 w-full min-w-[18rem] overflow-hidden rounded-2xl border border-emerald-200 bg-white shadow-xl sm:min-w-[26rem]">
              ...
              filtered.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-start justify-between gap-3 rounded-xl px-3 py-3 text-left transition ${active ? "bg-primary/10 text-primary" : "hover:bg-emerald-50/60"}`}
                  >
                    <div className="min-w-0">
                      <div className="truncate font-medium">
                        {option.displayLabel || option.value}
                      </div>
                      <div className="truncate text-sm text-muted-foreground">
                        {option.displayMeta || option.label}
                      </div>
                      {option.description ? (
                        <div className="truncate text-xs text-muted-foreground">
                          {option.description}
