...
export default function UsersManagement() {
  ...
  const pharmacistFunction = String(editForm.functionName || "")
    .trim()
    .toUpperCase();
  ...
  const openEditor = (item) => {
    ...
    setEditForm({
      ...
      assignedLocationId: item.assignedDepotId
        ? String(item.assignedDepotId)
        : "",
    });
  };
  const closeEditor = () => {
  ...
  const handleToggleStatus = async (item) => {
    ...
    try {
      ...
      setSuccessMessage(
        `${t("adminUsers.statusUpdatedFor")} ${updatedUser.firstname} ${updatedUser.lastname}.`,
      );
    } catch (err) {
  ...
  const handleDelete = async (item) => {
    ...
    try {
      ...
      if (result?.deleted) {
        ...
        setSuccessMessage(
          `${t("adminUsers.deletedFor")} ${item.firstname} ${item.lastname}.`,
        );
        ...
        setSuccessMessage(
          `${t("adminUsers.deactivatedFallbackFor")} ${result.user.firstname} ${result.user.lastname}.`,
        );
      } else {
        setUsers((current) => current.filter((entry) => entry.id !== item.id));
        if (editingUser?.id === item.id) {
      ...
      if (err?.response?.status === 404) {
        ...
        setSuccessMessage(
          `${t("adminUsers.alreadyDeletedFor")} ${item.firstname} ${item.lastname}.`,
        );
        return;
      }
      setError(
  ...
  const handleSave = async () => {
    ...
    const normalizedForm = {
      ...
      assignedLocationId: editForm.assignedLocationId
        ? Number(editForm.assignedLocationId)
        : null,
    };
    if (
    ...
    const rolesChanged =
      selectedRoles.length !== editingUser.roles.length ||
    ...
    if (pharmacistSelected) {
      ...
      if (
        depotRequired &&
        normalizedForm.assignedLocationId !==
          Number(editingUser.assignedDepotId || 0)
      ) {
    ...
    try {
      ...
      setEditForm({
        ...
        doctorSpecialty:
          updatedUser.doctorSpecialty || updatedUser.specialty || "",
        assignedLocationId: updatedUser.assignedDepotId
          ? String(updatedUser.assignedDepotId)
          : "",
      });
      setSelectedRoles(updatedUser.roles);
      setSuccessMessage(
        `${t("adminUsers.updatedFor")} ${updatedUser.firstname} ${updatedUser.lastname}.`,
      );
            ...
            <CardContent className="flex min-h-0 flex-1 flex-col overflow-hidden">
                ...
                <div className="grid gap-4 rounded-xl border bg-muted/20 p-4 md:grid-cols-2">
                  ...
                  {editingAsPharmacist ? (
                    <div>
                      ...
                      <select
                        className="mt-1 h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm shadow-sm outline-none focus:border-primary"
                  ...
                  {editingAsPharmacist ? (
                    <div>
                      ...
                      {showDepotField ? (
                        <select
                          className="mt-1 h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm shadow-sm outline-none focus:border-primary"
                          value={editForm.assignedLocationId}
                          onChange={(event) =>
                            setEditForm((current) => ({
                              ...current,
                              assignedLocationId: event.target.value,
                            }))
                          }
                          disabled={saving}
                        >
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(event) =>
                          setEditForm((current) => ({
                            ...current,
                            doctorSpecialty: event.target.value,
                          }))
                        }
                        disabled={saving}
                      />
              ...
              <div className="mt-6 flex shrink-0 justify-end gap-3 border-t border-slate-200 bg-white pt-4">
                ...
                <Button
                  ...
                  disabled={saving}
                >
                  {saving ? t("adminUsers.savingRoles") : t("common.save")}
                </Button>
              </div>
            </CardContent>
          </Card>
