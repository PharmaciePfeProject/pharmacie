...
export default function StockMovementDetails() {
    ...
    return (<div className="space-y-6">
      ...
      {item && !loading && !error && (<>
          <DetailHero eyebrow={t("page.movements.title")} title={String(item.num_movement || `Movement #${item.movement_id}`)} description={t("movementDetails.heroDescription")} badge={`${item.lines.length} ${item.lines.length === 1 ? t("common.line") : t("common.linesLabel")}`}/>

          <DetailStatGrid>
            <DetailStatCard label={t("movementDetails.date")} value={item.date_movement ? new Date(item.date_movement).toLocaleString() : t("common.notAvailable")} emphasis/>
            <DetailStatCard label={t("movementDetails.typeDiscriminator")} value={`${formatDetailValue(item.type_mvt, "-")} / ${formatDetailValue(item.descriminator, "-")}`}/>
          ...
          <DetailStatGrid>
            <DetailStatCard label={t("movements.referenceType")} value={`${formatDetailValue(item.reference_type_id, "-")} / ${formatDetailValue(item.reference_type_label, "-")}`}/>
            <DetailStatCard label={t("common.location")} value={`${formatDetailValue(item.location_id, "-")} / ${formatDetailValue(item.location_label, "-")}`}/>
            <DetailStatCard label={t("movementDetails.businessLinks")} value={<div className="space-y-1 text-sm font-medium text-slate-700">
          ...
          <DetailSection title={t("movementDetails.linesTitle")} description={t("movementDetails.linesDescription")}>
