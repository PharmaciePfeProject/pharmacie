...
export default function InventoriesList() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (active)
                    setError(err?.response?.data?.message || t("inventories.loading"));
            }
            finally {
                if (active)
                    setLoading(false);
    ...
    const applyFilters = () => {
        ...
        if (locationId.trim())
            next.set("location_id", locationId.trim());
        if (stateId.trim())
            next.set("state_id", stateId.trim());
        if (userId.trim())
    ...
    return (<div className="space-y-6">
      <div className="rounded-xl border bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex justify-end gap-3">
          <Button variant="outline" className="rounded-xl" onClick={() => { setProductId(""); setLocationId(""); setStateId(""); setUserId(""); setDateFrom(""); setDateTo(""); setSearchParams(new URLSearchParams({ page: "1", pageSize: String(currentPageSize) })); }}>{t("common.reset")}</Button>
          <Button className="rounded-xl" onClick={applyFilters}>{t("common.apply")}</Button>
        </div>
      </div>

      {loading && <div className="rounded-xl border bg-white p-8 text-center text-muted-foreground shadow-sm">{t("inventories.loading")}</div>}
            ...
            <table className="table-auto w-full border-collapse text-sm">
              <thead className="bg-muted/50">
                <tr className="text-left">
                  <th className="px-4 py-3 text-sm font-semibold">{t("inventories.inventoryId")}</th>
              ...
              <tbody>
                {items.map((item) => (<tr key={item.inventory_id} className="border-t hover:bg-gray-50">
                    <td className="px-4 py-3 font-medium">{item.inventory_id}</td>
                    <td className="px-4 py-3">{item.date_inv ? new Date(item.date_inv).toLocaleString() : t("common.notAvailable")}</td>
                    ...
                    <td className="px-4 py-3"><Link to={`/app/inventories/${item.inventory_id}`}><Button size="sm" variant="outline" className="rounded-xl">{t("common.view")}</Button></Link></td>
                  </tr>))}
              </tbody>
            </table>
          </div>
