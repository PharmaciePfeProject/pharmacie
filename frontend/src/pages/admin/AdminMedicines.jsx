...
export default function AdminMedicines() {
    ...
    const [dciOptions, setDciOptions] = useState([]);
    const [pharmaClasses, setPharmaClasses] = useState([]);
    const [productTypes, setProductTypes] = useState([]);
    const [editingProductId, setEditingProductId] = useState(null);
    const canSubmit = useMemo(() => {
        return Boolean(form.lib.trim() &&
            form.bar_code.trim() &&
            form.dci.trim() &&
            form.pharma_class_id &&
            form.type_id);
    }, [form]);
    const dciPickerOptions = useMemo(() => dciOptions.map((item) => ({
        value: String(item.dci_id),
        label: item.label || "-",
    })), [dciOptions]);
    const pharmaClassOptions = useMemo(() => pharmaClasses.map((item) => ({
        value: String(item.pharma_class_id),
        label: item.label || "-",
    })), [pharmaClasses]);
    const productTypeOptions = useMemo(() => productTypes.map((item) => ({
    ...
    return (<div className="space-y-6">
      <div className="rounded-xl border bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          ...
          {editingProductId ? (<Link to="/app/admin/medicines">
        ...
        <form onSubmit={onSubmit} className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="space-y-2">
            ...
            <PickerField value={selectedDciValue} onChange={(value) => {
            const selected = dciOptions.find((item) => String(item.dci_id) === value);
            updateField("dci", selected?.label || "");
        }} options={dciPickerOptions} placeholder={t("adminMedicines.select")} searchPlaceholder={t("adminMedicines.searchDci")} emptyText={t("adminMedicines.noOption")} helperText={t("adminMedicines.dciHelp")} compact/>
          </div>
          ...
          <div className="space-y-2">
            ...
            <PickerField value={form.pharma_class_id} onChange={(value) => updateField("pharma_class_id", value)} options={pharmaClassOptions} placeholder={t("adminMedicines.select")} searchPlaceholder={t("adminMedicines.searchPharmaClass")} emptyText={t("adminMedicines.noOption")} helperText={t("adminMedicines.pharmaClassHelp")} compact/>
          </div>

          <div className="space-y-2">
            <p className="text-sm font-medium">{t("adminMedicines.productType")}</p>
            <PickerField value={form.type_id} onChange={(value) => updateField("type_id", value)} options={productTypeOptions} placeholder={t("adminMedicines.select")} searchPlaceholder={t("adminMedicines.searchProductType")} emptyText={t("adminMedicines.noOption")} helperText={t("adminMedicines.productTypeHelp")} compact/>
          </div>
      ...
      <div className="rounded-xl border bg-white p-5 shadow-sm">
        ...
        {loadingProducts ? (<p className="mt-4 text-sm text-muted-foreground">{t("common.loading")}</p>) : (<div className="mt-4 overflow-x-auto">
            <table className="w-full table-auto border-collapse text-sm">
              ...
              <tbody>
                {products.map((item) => (<tr key={item.product_id} className="border-t">
