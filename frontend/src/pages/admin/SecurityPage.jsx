...
export default function SecurityPage() {
    ...
    const analyticsUsers = useMemo(() => users.filter((item) => item.permissions.includes(PERMISSIONS.ANALYTICS_READ)), [users]);
    ...
    return (<div className="space-y-6">
      <Card className="border-white/70 bg-white/95 shadow-sm">
        ...
        <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[
            {
                label: t("security.totalUsers"),
                value: users.length,
                icon: Users,
            },
            {
                label: t("security.activeUsers"),
                value: activeUsers.length,
                icon: ShieldCheck,
            },
            {
                label: t("security.adminUsers"),
                value: adminUsers.length,
                icon: UserCog,
            },
            {
                label: t("security.doctorUsers"),
                value: doctorUsers.length,
                icon: Stethoscope,
            },
        ].map(({ label, value, icon: Icon }) => (<div key={label} className="rounded-3xl border border-border bg-muted/20 p-5">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
          ...
          <CardContent className="space-y-5">
            ...
            <div>
              <p className="mb-3 text-sm font-medium">
                {t("security.currentRoles")}
              </p>
              <div className="flex flex-wrap gap-2">
                {sessionRoles.length > 0 ? (sessionRoles.map((role) => (<span key={role} className="rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
            ...
            <div>
              <p className="mb-3 text-sm font-medium">
                {t("security.currentPermissions")}
              </p>
              <div className="flex flex-wrap gap-2">
                {sessionPermissions.length > 0 ? (sessionPermissions.map((permission) => (<span key={permission} className="rounded-full bg-muted px-3 py-1 text-xs text-muted-foreground">
                      {permission}
                    </span>))) : (<span className="text-sm text-muted-foreground">
                    {t("common.notAvailable")}
                  </span>)}
        ...
        <Card className="border-white/70 bg-white/95 shadow-sm">
          ...
          <CardContent className="space-y-3">
            ...
            {
                to: "/app/admin/users",
                label: t("security.action.users"),
                icon: Users,
            },
            {
                to: "/app/bi/reports",
                label: t("security.action.analytics"),
                icon: ChartColumnBig,
            },
        ].map(({ to, label, icon: Icon }) => (<Button key={to} asChild variant="outline" className="h-auto w-full justify-start gap-3 rounded-2xl px-4 py-4">
                <Link to={to}>
                  <Icon className="h-4 w-4"/>
                  <span>{label}</span>
                </Link>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Card className="border-white/70 bg-white/95 shadow-sm">
          ...
          <CardContent>
            {loading ? (<div className="rounded-xl border bg-muted/20 p-6 text-sm text-muted-foreground">
                {t("security.loading")}
              </div>) : error ? (<div className="rounded-xl border border-destructive/20 bg-destructive/5 p-6 text-sm text-destructive">
                {error}
              </div>) : roleDistribution.length === 0 ? (<EmptyState description={t("security.noRoles")}/>) : (<div className="space-y-3">
                {roleDistribution.map(([role, count]) => (<div key={role} className="flex items-center justify-between rounded-2xl border bg-muted/20 px-4 py-3">
                    <div>
                      <p className="font-medium">{formatRoleLabel(role)}</p>
                      <p className="text-sm text-muted-foreground">
                        {t("security.accountsWithRole")}
                      </p>
                    </div>
        ...
        ].map((item) => (<div key={item} className="flex items-start gap-3 rounded-2xl border bg-muted/20 p-4">
