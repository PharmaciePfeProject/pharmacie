...
import { AlertTriangle, PencilLine, Power, Search, ShieldCheck, Trash2, UserCog, X } from "lucide-react";
import {
  deleteManagedUser,
  fetchUsers,
  updateManagedUser,
...
export default function UsersManagement() {
    ...
    useEffect(() => {
      setCurrentPage((page) => Math.min(page, Math.max(1, Math.ceil(filteredUsers.length / pageSize))));
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="bg-muted/50">
              <tr className="text-left">
                ...
                <th className="px-4 py-3 text-sm font-semibold">{t("common.status")}</th>
                <th className="px-4 py-3 text-sm font-semibold">
            ...
            <tbody>
              {paginatedUsers.map((item) => (<tr key={item.id} className="border-t hover:bg-gray-50">
                ...
                : "bg-slate-200 text-slate-600"}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-2">
                      <Button size="sm" variant="outline" className="rounded-xl border-emerald-300 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 hover:text-emerald-800" onClick={() => openEditor(item)}>
                        <PencilLine className="mr-1 h-4 w-4" />
                        {t("common.edit")}
                      </Button>
                      <Button size="sm" variant="outline" className={`rounded-xl ${item.actived === 1 ? "border-amber-300 bg-amber-50 text-amber-700 hover:bg-amber-100 hover:text-amber-800" : "border-blue-300 bg-blue-50 text-blue-700 hover:bg-blue-100 hover:text-blue-800"}`} onClick={() => handleToggleStatus(item)} disabled={busyUserId === item.id || item.id === currentUser?.id}>
                        <Power className="mr-1 h-4 w-4" />
                        {item.actived === 1 ? t("common.deactivate") : t("common.activate")}
                      </Button>
                      <Button size="sm" variant="destructive" className="rounded-xl bg-rose-600 text-white hover:bg-rose-700" onClick={() => setUserToDelete(item)} disabled={busyUserId === item.id || item.id === currentUser?.id}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-rose-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-rose-100 px-6 py-4">
              <h3 className="text-lg font-semibold text-slate-900">{t("common.delete")}</h3>
              <button type="button" className="rounded-xl p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-700" onClick={() => setUserToDelete(null)}>
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-slate-700">{t("adminUsers.deleteConfirm")}</p>
              <p className="mt-2 text-xs text-slate-500">
                {userToDelete.firstname} {userToDelete.lastname} ({userToDelete.username})
              </p>
            </div>
            <div className="flex justify-end gap-2 border-t border-slate-200 px-6 py-4">
              <Button type="button" variant="outline" onClick={() => setUserToDelete(null)} disabled={busyUserId !== null}>
                {t("common.cancel")}
              </Button>
              <Button type="button" className="bg-rose-600 text-white hover:bg-rose-700" onClick={() => handleDelete(userToDelete)} disabled={busyUserId !== null}>
                <Trash2 className="mr-1 h-4 w-4" />
                {busyUserId ? t("adminUsers.savingRoles") : t("common.delete")}
              </Button>
            </div>
          </div>
        </div>
