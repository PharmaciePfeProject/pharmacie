...
export default function UserRegistration() {
    ...
    const roleOptions = [
      { value: "MEDECIN", label: isEnglish ? "Doctor" : "Medecin" },
      { value: "PHARMACIEN", label: isEnglish ? "Pharmacist" : "Pharmacien" },
      { value: "RESPONSABLE_REPORTING", label: isEnglish ? "BI Manager" : "Responsable BI" },
      { value: "SECRETAIRE_GENERAL", label: isEnglish ? "General Secretary" : "Secretaire general" },
    ];
    const pharmacistFunctionOptions = [
      { value: "PRESCRIPTIONS", label: isEnglish ? "Prescriptions" : "Prescriptions" },
      { value: "DEPOT", label: isEnglish ? "Depot" : "Depot" },
    ];
      ...
      <Card className="border-white/70 bg-white/95 shadow-sm">
        ...
        <CardContent>
          <form onSubmit={submit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium">{isEnglish ? "Role" : "Role"}</p>
            ...
            {form.role === "PHARMACIEN" ? (<div className="space-y-2 md:col-span-2">
                <p className="text-sm font-medium">{isEnglish ? "Pharmacist function" : "Fonction du pharmacien"}</p>
                <select className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm shadow-sm outline-none focus:border-primary" value={form.functionName} onChange={(event) => updateField("functionName", event.target.value)}>
                  <option value="">{isEnglish ? "Select a function" : "Selectionner une fonction"}</option>
            ...
            {mustChooseDepot ? (<div className="space-y-2 md:col-span-2">
                <p className="text-sm font-medium">{isEnglish ? "Pharmacist depot" : "Depot du pharmacien"}</p>
                <select className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm shadow-sm outline-none focus:border-primary" value={form.assignedLocationId} onChange={(event) => updateField("assignedLocationId", event.target.value)}>
                  <option value="">{isEnglish ? "Select a depot" : "Selectionner un depot"}</option>
                  {locations.map((location) => (<option key={location.location_id} value={location.location_id}>
                      {location.location_id} / {location.lib || (isEnglish ? "Depot" : "Depot")}
                    </option>))}
                </select>
            ...
            {form.role === "MEDECIN" ? (<div className="space-y-2 md:col-span-2">
                <p className="text-sm font-medium">{isEnglish ? "Doctor specialty" : "Specialite du medecin"}</p>
                <Input value={form.doctorSpecialty} onChange={(event) => updateField("doctorSpecialty", event.target.value)} placeholder={isEnglish ? "Example: Cardiology" : "Exemple: Cardiologie"}/>
              </div>) : null}
