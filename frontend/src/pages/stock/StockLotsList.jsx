...
export default function StockLotsList() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (active)
                    setError(err?.response?.data?.message || t("stockLots.loading"));
            }
            finally {
                if (active)
                    setLoading(false);
    ...
    return (<div className="space-y-6">
      ...
      {loading && <div className="rounded-xl border bg-white p-8 text-center text-muted-foreground shadow-sm">{t("stockLots.loading")}</div>}
            ...
            <table className="table-auto w-full border-collapse text-sm">
              <thead className="bg-muted/50">
                <tr className="text-left">
                  <th className="px-4 py-3 text-sm font-semibold">{t("stockLots.stockLotId")}</th>
                  <th className="px-4 py-3 text-sm font-semibold">{t("common.product")}</th>
                  <th className="px-4 py-3 text-sm font-semibold">{t("stockLots.lotId")}</th>
                  <th className="px-4 py-3 text-sm font-semibold">{t("stockLots.lotLabel")}</th>
                  <th className="px-4 py-3 text-sm font-semibold">{t("stockLots.state")}</th>
                  <th className="px-4 py-3 text-sm font-semibold">{t("stockLots.dateRefusal")}</th>
              ...
              <tbody>
                {items.map((item) => (<tr key={item.stock_lot_id} className="border-t hover:bg-gray-50">
                    <td className="px-4 py-3 font-medium">{item.stock_lot_id}</td>
                    <td className="px-4 py-3">{item.product_lib || t("common.notAvailable")}</td>
                    <td className="px-4 py-3">{item.lot_id ?? t("common.notAvailable")}</td>
                    <td className="px-4 py-3">{item.lot_label || t("common.notAvailable")}</td>
                    <td className="px-4 py-3">{item.lot_state ?? t("common.notAvailable")}</td>
                    <td className="px-4 py-3">
                      {item.date_refusal ? new Date(item.date_refusal).toLocaleDateString() : t("common.notAvailable")}
                    </td>
                    <td className="px-4 py-3">{item.quantity}</td>
                    <td className="px-4 py-3">
                      {item.emplacement_id ?? t("common.notAvailable")} / {item.emplacement_label || t("common.notAvailable")}
                    </td>
                  </tr>))}
              </tbody>
            </table>
