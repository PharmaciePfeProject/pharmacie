...
export default function InternalDeliveryDetails() {
    ...
    return (<div className="space-y-6">
      ...
      {item && !loading && !error && (<>
          <DetailHero eyebrow="Distribution" title={String(item.delivery_number || `Internal delivery #${item.internal_delivery_id}`)} description="Use this view to understand destination, linked internal order, and missing quantities without digging through a dense table first." badge={`${item.lines.length} line${item.lines.length === 1 ? "" : "s"}`}/>
          <DetailStatGrid>
