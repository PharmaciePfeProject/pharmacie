...
export default function ReceptionsList() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (active)
                    setError(err?.response?.data?.message || t("receptions.loading"));
            }
            finally {
                if (active)
                    setLoading(false);
    ...
    const applyFilters = () => {
        ...
        if (externalOrderId.trim())
            next.set("external_order_id", externalOrderId.trim());
        if (dateFrom.trim())
      ...
      {!loading && !error && items.length > 0 && (<>
          <div className="overflow-x-auto rounded-xl border bg-white shadow-sm">
            <table className="table-auto w-full border-collapse text-sm">
              ...
              <tbody>
                {items.map((item) => (<tr key={item.reception_id} className="border-t hover:bg-gray-50">
