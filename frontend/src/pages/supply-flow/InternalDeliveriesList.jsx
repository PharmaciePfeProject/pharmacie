...
export default function InternalDeliveriesList() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (active)
                    setError(err?.response?.data?.message || t("internalDeliveries.loading"));
            }
            finally {
                if (active)
                    setLoading(false);
    ...
    const applyFilters = () => {
        ...
        if (customerId.trim())
            next.set("customer_id", customerId.trim());
        if (internalOrderId.trim())
            next.set("internal_order_id", internalOrderId.trim());
        if (dateFrom.trim())
      ...
      {!loading && !error && items.length > 0 && (<>
          <div className="overflow-x-auto rounded-xl border bg-white shadow-sm">
            <table className="table-auto w-full border-collapse text-sm">
              ...
              <tbody>
                {items.map((item) => (<tr key={item.internal_delivery_id} className="border-t hover:bg-gray-50">
