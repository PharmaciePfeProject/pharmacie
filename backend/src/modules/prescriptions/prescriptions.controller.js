...
export async function updatePrescription(req, res) {
  ...
  try {
    const existingPrescriptionRes = await conn.execute(
      `SELECT ID FROM ${PRESCRIPTION_TABLE} WHERE ID = :id FOR UPDATE`,
      { id: prescriptionId }
    );
    if (existingPrescriptionRes.rows.length === 0) {
      return res.status(404).json({ message: "Prescription not found" });
    }

    const approvalRes = await conn.execute(
      `SELECT STATUS FROM ${PRESCRIPTION_APPROVAL_TABLE} WHERE PRESCRIPTION_ID = :id FOR UPDATE`,
      { id: prescriptionId }
    );
    const status = approvalRes.rows[0]?.STATUS || "PENDING";
    if (status !== "PENDING") {
      return res.status(409).json({
        message: "Only pending prescriptions can be edited.",
      });
    }
    ...
    await conn.execute(
      `DELETE FROM ${PRESCRIPTION_LINE_TABLE} WHERE PRESCRIPTION_ID = :prescription_id`,
      { prescription_id: prescriptionId },
      { autoCommit: false }
    );
    ...
    await conn.execute(
      `DELETE FROM ${PRESCRIPTION_REQUEST_TABLE} WHERE PRESCRIPTION_ID = :prescription_id`,
      { prescription_id: prescriptionId },
      { autoCommit: false }
    );
    ...
    for (const radioLabel of radioRequests) {
      await conn.execute(
        `INSERT INTO ${PRESCRIPTION_REQUEST_TABLE} (
          ID, PRESCRIPTION_ID, REQUEST_TYPE, REQUEST_LABEL, REQUEST_NOTES, CREATED_AT
        ) VALUES (
          :id, :prescription_id, 'RADIO', :request_label, NULL, SYSTIMESTAMP
        )`,
    ...
    for (const analysisLabel of analysisRequests) {
      await conn.execute(
        `INSERT INTO ${PRESCRIPTION_REQUEST_TABLE} (
          ...
          :id, :prescription_id, 'ANALYSIS', :request_label, NULL, SYSTIMESTAMP
        )`,
...
export async function deletePrescription(req, res) {
  ...
  try {
    ...
    await conn.execute(`DELETE FROM ${PRESCRIPTION_REQUEST_TABLE} WHERE PRESCRIPTION_ID = :id`, { id: prescriptionId }, { autoCommit: false });
    await conn.execute(`DELETE FROM ${PRESCRIPTION_LINE_TABLE} WHERE PRESCRIPTION_ID = :id`, { id: prescriptionId }, { autoCommit: false });
    await conn.execute(`DELETE FROM ${PRESCRIPTION_APPROVAL_TABLE} WHERE PRESCRIPTION_ID = :id`, { id: prescriptionId }, { autoCommit: false });
    await conn.execute(`DELETE FROM ${PRESCRIPTION_TABLE} WHERE ID = :id`, { id: prescriptionId }, { autoCommit: false });
    await conn.commit();
  } catch (error) {
    await conn.rollback();
    throw error;
